...
public class StatusViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenShipCanceled_then_UPDATE_2(@Payload ShipCanceled shipCanceled) {
        try {
            if (!shipCanceled.validate()) return;
                // view 객체 조회
            Optional<Status> statusOptional = statusRepository.findById(shipCanceled.getOrderId());
